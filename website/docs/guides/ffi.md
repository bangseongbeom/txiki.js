...
```javascript
import { toUint8Array, toArrayBuffer } from 'tjs:ffi';

const p = symbols.get_pixels();          // NativePointer to width*height*4 bytes
const pixels = toUint8Array(p, w * h * 4); // a view, not a copy

pixels[0] = 255;                          // writes straight to the native buffer
```

You can also call the methods directly on a pointer:

```javascript
const view = p.toUint8Array(len);          // or p.toArrayBuffer(len, offset)
```
...
got a pointer into — pass it as `owner` so the garbage collector can't reclaim it
...
After you free native memory, any views over it are dangling. Use
[`detachBuffer`](/docs/api/tjs-ffi.Function.detachBuffer) to neutralize a view so
later access throws or reads empty instead of touching freed memory:

```javascript
import { toArrayBuffer, detachBuffer } from 'tjs:ffi';

const buf = toArrayBuffer(p, len);
// ... use buf ...
symbols.free(p);
detachBuffer(buf); // buf.byteLength is now 0, buf.detached is true
```

Unlike `ArrayBuffer.prototype.transfer()`, `detachBuffer` does not read or copy
the bytes, so it is safe to call *after* the memory is gone. For a view returned
as a `Uint8Array`, pass its `.buffer`.
